document.addEventListener('DOMContentLoaded', () => {
  ...
  const timeoutMs = 60000;   // Fetch timeout
  ...
  // --- User Camera Interaction Tracking ---
  let userInteracted = false;
  let userInteractionTimeout = null;
  
  // Listen for user camera adjustments.
  // Whenever the camera is manually changed, mark the flag.
  modelViewer.addEventListener('camera-change', () => {
    userInteracted = true;
    clearTimeout(userInteractionTimeout);
    // Reset the flag after 5 seconds of inactivity.
  ...
  modelViewer.addEventListener('load', () => {
    ...
    // Only reset the camera view if the user hasn't manually adjusted it.
    if (!userInteracted) {
